...
import { Routes, Route, NavLink, useLocation, useNavigate } from 'react-router-dom'; // Import useNavigate
...
function App() {
  ...
  // New useEffect to handle post-payment cart clearing
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const pesapalTrackingId = params.get('pesapal_transaction_tracking_id');
    const pesapalResponseData = params.get('pesapal_response_data');

    // Check if Pesapal redirect parameters are present
    if (pesapalTrackingId && pesapalResponseData) {
      console.log('Payment success detected!', { pesapalTrackingId, pesapalResponseData });
      // Optionally, you might want to send these to your backend
      // for final payment verification before clearing the cart.

      // Clear the cart
      setCart([]);
      Cookies.remove('shoppingCart'); // Ensure cookie is also cleared
  ...
  }, [location.search, navigate, setCart]); // Rerun when search params change or navigate function changes
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-4 font-inter">
      ...
      <Routes>
        ...
        />
        {/* You might want a dedicated payment success page */}

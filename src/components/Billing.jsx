...
import { useNavigate } from 'react-router-dom'; // Import useNavigate for potential redirection back
...
function Billing({ cart, products }) {
  ...
  // Calculate the total price of all items in the cart (re-calculated here for direct use)
    ...
    <main className="w-full max-w-3xl bg-white p-8 rounded-xl shadow-2xl mx-auto my-8">
      ...
      {cart.length === 0 ? (
        <p className="text-gray-600 text-center text-lg">
          ...
          <br />
          <button
            onClick={() => navigate('/')} // Assuming your cart is at the root or another defined path
            ...
            <form onSubmit={initiatePayment} className="space-y-4">
              ...
              <button
                ...
                className={`w-full py-3 px-4 rounded-md text-white font-semibold transition-colors duration-200 ${
                  (isLoading || isIpnLoading || ipnError || !fetchedNotificationId || cart.length === 0) ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'
                }`}
              >
              ...
              <button
                ...
                onClick={() => navigate(-1)} // Go back to the previous page (Cart)
